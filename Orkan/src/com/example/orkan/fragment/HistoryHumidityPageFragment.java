...
import com.example.orkan.fragment.HistoryFragment.ProbeHandler;
...
public class HistoryHumidityPageFragment extends Fragment implements View.OnClickListener{
	...
	public void onCreate(Bundle savedInstanceState) {
     ...
     private void initData(){
 		...
 		for(int i=0;i<Util.POINTS_INDOOR_HUMIDITY.size();i++) {
 			float tpm = Util.POINTS_INDOOR_HUMIDITY.get(i).y;
 			if(max < tpm){
 				max = tpm;
 			}
 		}
 		float maxInt = (((int)max)/10+1)*10;
 		if (Util.POINTS_INDOOR_HUMIDITY.size()>0){
 			bezierCurveChart.init(Util.POINTS_INDOOR_HUMIDITY ,null, 
     		...
     		case Util.INSTANCE_CODE_HISTORY_REV:
     			try {
     				...
     				for(int i=0;i<humidityArray.length();i++) {
     					float tpm = Float.parseFloat(humidityArray.getString(i));
     					Util.POINTS_INDOOR_HUMIDITY.add(new Point(i, tpm));
     					if(max < tpm){
     						max = tpm;
     					}
     				}
