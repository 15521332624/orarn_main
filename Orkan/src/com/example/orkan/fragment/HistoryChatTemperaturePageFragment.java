...
public class HistoryChatTemperaturePageFragment extends Fragment implements View.OnClickListener{
...
private LineData getLineData() {  
	  ...
	  dataLenth = Util.CHAR_INDOOR_TEMPERATURE.size();
	  ArrayList<String> xVals = new ArrayList<String>();  
	  for (int i = 0; i < Util.CHAR_INDOOR_TEMPERATURE.size(); i++) {  
	      xVals.add(Util.CHAR_INDOOR_TEMPERATURE.get(i).time);  
	  }  
	 //hour and day add more point
	  if(dataLenth == 24 || dataLenth == 30)
		  xVals.add(Util.CHAR_INDOOR_TEMPERATURE.get(Util.CHAR_INDOOR_TEMPERATURE.size() - 1).time);  
	  
	  ArrayList<Entry> yVals = new ArrayList<Entry>();  
	  for (int i = 0; i < Util.CHAR_INDOOR_TEMPERATURE.size(); i++) {    
	      yVals.add(new Entry(Util.CHAR_INDOOR_TEMPERATURE.get(i).value, i));  
	  }
	  //hour and day add more point
	  if(dataLenth == 24 || dataLenth == 30)
		  yVals.add(new Entry(Util.CHAR_INDOOR_TEMPERATURE.get(Util.CHAR_INDOOR_TEMPERATURE.size() - 1).value, Util.CHAR_INDOOR_TEMPERATURE.size()));  
