...
public class SystemUtils {
	 ...
	 * @return
	 */
	public static List<CityDB> getTmpCities(Cursor c) {
		...
		while (c.moveToNext()) {
			...
			CityDB item = new CityDB(name, postID, refreshTime, isLocation, pubTime, weatherInfoStr);
	 ...
	 * @return
	 */
	public static List<CityDB> getHotCities(Cursor c) {
		List<CityDB> list = new ArrayList<CityDB>();
		if (c == null || c.getCount() == 0)
			return list;
		while (c.moveToNext()) {
			...
			String postID = c
					.getString(c.getColumnIndex(CityConstants.POST_ID));
			CityDB item = new CityDB(name, postID);
			list.add(item);
		}
		c.close();
		return list;
	 ...
	 * @return
	 */
	public static List<CityDB> getAllCities(Cursor c) {
		List<CityDB> list = new ArrayList<CityDB>();
		if (c == null || c.getCount() == 0)
			return list;
		while (c.moveToNext()) {
			...
			String postID = c
					.getString(c.getColumnIndex(CityConstants.POST_ID));
			CityDB item = new CityDB(province, city, name, pinyin, py, phoneCode,
					areaCode, postID);
			list.add(item);
		}
		c.close();
